...
@interface GlkPairWindow : GlkWindow {
	...
	// Arrangement options
	/// Proportional arrangement if `NO`.
	BOOL fixed;
	/// Vertical arrangement if `NO`.
	BOOL horizontal;
	/// `NO` if left is above/left of right, `YES` otherwise.
	BOOL above;
...
/// Set to `YES` to only draw the border if input is requested.
