...
@interface GlkTextWindow : GlkWindow <NSTextStorageDelegate, GlkTextViewDelegate> {
	...
	/// `YES` if the buffer is flushing.
	BOOL flushing;
	/// `YES` if the buffer is echoing.
	BOOL lineEcho;
	
	/// `YES` if a request to make the text editable is pending.
	BOOL willMakeEditable;
	/// `YES` if a request to make the text non-editable is pending.
	BOOL willMakeNonEditable;
	
	/// `YES` if this window has a more prompt
	BOOL hasMorePrompt;
