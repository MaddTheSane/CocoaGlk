...
@interface GlkView : NSView<GlkSession, GlkBuffer, GlkEventReceiver> {
	...
	NSMutableDictionary<NSNumber*,NSMutableDictionary*>* styles;								//!< Active styles
	CGFloat scaleFactor;										//!< The active scale factor
	...
	// The task
	BOOL running;												//!< \c YES if the task is running
	NSString* viewCookie;										//!< The session cookie to use with this view
	NSTask* subtask;											//!< Only used if this is connected as a session via the \c launchClientApplication: method
	
	// The delegate
	id<GlkViewDelegate> delegate;								// Can respond to certain events if it likes
	...
	NSInteger historyPosition;									//!< Current history position
...
// The delegate
@property (assign) id<GlkViewDelegate> delegate;				//!< The delegate for this view. Delegates are not retained.
...
// Preferences
@property (retain) GlkPreferences* preferences;					//!< Set before this view has attached to a client for the best effect
