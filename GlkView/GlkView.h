...
@interface GlkView : GlkSuperView<GlkSession, GlkBuffer, GlkEventReceiver> {
...
#if !defined(COCOAGLK_IPHONE)
	/// Only used if this is connected as a session via the `launchClientApplication:` method.
	NSTask* subtask;
#endif
	...
	NSMutableDictionary<NSNumber*,GlkWindow*>* windowIdCache;
}
...
/// The delegate can override this to provide custom saving behaviour for its files. This should return `YES` if the delegate is going to handle the event or `NO` otherwise.
